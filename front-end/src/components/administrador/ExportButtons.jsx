...
export default function ExportButtons() {
  ...
  const handleExportarExcel = async () => {
    try {
      ...
      showInfo('Iniciando geração do relatório Excel...', 2000);
      
      const resultado = await exportarParaExcel();
      showSuccess(`Relatório Excel gerado com sucesso! Arquivo: ${resultado.arquivo}`, 4000);
    } catch (error) {
  ...
  const handleExportarPDF = async () => {
    try {
      ...
      showInfo('Iniciando geração do relatório PDF...', 2000);
      
      const resultado = await exportarParaPDF();
      showSuccess(`Relatório PDF gerado com sucesso! Arquivo: ${resultado.arquivo}`, 4000);
    } catch (error) {
